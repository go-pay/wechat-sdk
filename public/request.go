...
func (s *SDK) doRequestUpload(c context.Context, path, fieldName string, file *bm.File, ptr any) (res *http.Response, err error) {
	uri := s.Host + path
	if s.DebugSwitch == wechat.DebugOn {
		s.logger.Debugf("Wechat_Public_SDK_URI: %s", uri)
	}
	req := s.hc.Req()
	req.Header.Add(wechat.HeaderRequestID, fmt.Sprintf("%s-%d", util.RandomString(21), time.Now().Unix()))
	req.Post(uri).SendMultipartBodyMap(bm.BodyMap{fieldName: file})
	res, bs, err := req.EndBytes(c)
	if err != nil {
		return nil, fmt.Errorf("http.request(POST, %s), err:%w", uri, err)
	}
	if s.DebugSwitch == wechat.DebugOn {
...
func (s *SDK) doRequestUploadWithForm(c context.Context, path, fieldName string, file *bm.File, formField string, formData bm.BodyMap, ptr any) (res *http.Response, err error) {
	uri := s.Host + path
	if s.DebugSwitch == wechat.DebugOn {
		s.logger.Debugf("Wechat_Public_SDK_URI: %s", uri)
	}
	req := s.hc.Req()
	req.Header.Add(wechat.HeaderRequestID, fmt.Sprintf("%s-%d", util.RandomString(21), time.Now().Unix()))
	body := bm.BodyMap{fieldName: file}
	body.Set(formField, formData.JsonBody())
	req.Post(uri).SendMultipartBodyMap(body)
	res, bs, err := req.EndBytes(c)
	if err != nil {
		return nil, fmt.Errorf("http.request(POST, %s), err:%w", uri, err)
	}
...
func (s *SDK) doRequestGetMedia(c context.Context, path string) (media []byte, err error) {
	uri := s.Host + path
	if s.DebugSwitch == wechat.DebugOn {
		s.logger.Debugf("Wechat_Public_SDK_URI: %s", uri)
	}
	...
	if s.DebugSwitch == wechat.DebugOn {
		s.logger.Debugf("Wechat_Public_SDK_Response: [%d] -> %d bytes", res.StatusCode, len(bs))
	}
	return bs, nil
}
